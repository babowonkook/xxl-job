(function ($) {
    ...
    $.fn.extend({
        cronGen: function (options) {
            ...
            $('<li/>').html($('<a id="WeeklyTab" href="#Weekly">요일</a>')).appendTo(topMenu);
            $('<li/>').html($('<a id="YearlyTab" href="#Yearly">연도</a>')).appendTo(topMenu);
            ...
            $(yearly1).append("지정하지 않음 사용가능한 특수문자[, - * /] 필수입력 아님");
            ...
            // Add an input
            var $i = $("<input>", { type: 'text', placeholder: 'cron표현식...', name: 'cronGen_display' }).addClass("form-control").val($(that).val());
            $i.appendTo($g);
            // Add the button
